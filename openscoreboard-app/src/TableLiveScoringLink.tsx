                        ...
                        <View maxW={"lg"} width={"100%"} alignSelf="center">
                            ...
                            {Object.entries(selectedTables).filter(([id, isSelected]) => {
                                ...
                                <View padding={1} flexDir={"row"}>
                                    <Input flex={1} isReadOnly value={`${window.location.origin}${subFolderPath}/live-scores?tables=${Object.entries(selectedTables).filter(([id, isSelected]) => {
                                        return isSelected
                                    }).map(([id, isSelected]) => {
                                        return id
                                    }).join(",")}`}></Input>
                                    <CopyButton text={`${window.location.origin}${subFolderPath}/live-scores?tables=${Object.entries(selectedTables).filter(([id, isSelected]) => {
                                        return isSelected
                                    }).map(([id, isSelected]) => {
                                        return id
                                    }).join(",")}`} />
